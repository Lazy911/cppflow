...
void Tensor::set_data(std::vector<T> new_data) {
    ...
    //lwj 这一句，因为对于构建python时候tf.bool没有形状，所以屏蔽掉就好了。
	...
	//lwj：修改了， 关于这部分还要研究学习，指针方面的东西。
	memcpy(this->data, &new_data[0], sizeof(T) * new_data.size());
...
TF_DataType Tensor::deduce_type() {
    ...
    //    return TF_BOOL;
	if (std::is_same<T, bool>::value)  //lwj
		return TF_BOOL;
...
template TF_DataType Tensor::deduce_type<bool>(); //lwj
...
template void Tensor::set_data<bool>(std::vector<bool> new_data); //lwj
...
template void Tensor::set_data<bool>(std::vector<bool> new_data, const std::vector<int64_t>& new_shape); //lwj
